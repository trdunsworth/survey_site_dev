...
describe('server API routes', () => {
    ...
    it('issues token and attempts email when resume email is supplied', async () => {
        ...
        expect(response.status).toBe(200);
        expect(deps.issueResumeToken).toHaveBeenCalledWith('sub-1', 'default', 1, expect.objectContaining({ requestedEmail: 'user@example.com' }));
        expect(deps.sendResumeTokenEmail).toHaveBeenCalledWith('user@example.com', 't1', expect.stringContaining('/?t=t1'), '2026-01-01T00:00:00.000Z');
